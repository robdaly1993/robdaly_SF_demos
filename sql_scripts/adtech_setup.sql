...
create or replace semantic view ADTECH_AI_DEMO.DEMO_SCHEMA.ADTECH_QUALITY_SEMANTIC_VIEW
  ...
  facts (
    ...
    IMPRESSIONS.BLOCKED_IMPRESSIONS   as blocked_impressions,
    IMPRESSIONS.FRAUD_IMPRESSIONS     as fraud_impressions,
    IMPRESSIONS.UNSAFE_IMPRESSIONS    as unsafe_impressions,
    IMPRESSIONS.MEDIA_COST            as media_cost
  )
  dimensions (
    IMPRESSIONS.RECORD_ID    as RECORD_ID,
    IMPRESSIONS.DATE         as date,
    IMPRESSIONS.RECORD_MONTH as MONTH(IMPRESSIONS.DATE),
    IMPRESSIONS.RECORD_YEAR  as YEAR(IMPRESSIONS.DATE),
  ...
  metrics (
    ...
    IMPRESSIONS.BLOCK_RATE        as (SUM(IMPRESSIONS.BLOCKED_IMPRESSIONS) / NULLIF(SUM(IMPRESSIONS.MEASURED_IMPRESSIONS), 0)),
    IMPRESSIONS.FRAUD_RATE        as (SUM(IMPRESSIONS.FRAUD_IMPRESSIONS) / NULLIF(SUM(IMPRESSIONS.MEASURED_IMPRESSIONS), 0)),
    IMPRESSIONS.UNSAFE_RATE       as (SUM(IMPRESSIONS.UNSAFE_IMPRESSIONS) / NULLIF(SUM(IMPRESSIONS.MEASURED_IMPRESSIONS), 0)),
